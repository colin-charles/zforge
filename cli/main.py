...
from cli._config import load_credentials, CONFIG_PATH
...
def login(
    ...
    from .auth import login_with_token, login_manual, login_browser_oauth
...
def build(
    ...
    tags: str = typer.Option("automation", "--tags", "-t", help="Comma-separated tags e.g. pdf,text,web"),
    category: str = typer.Option("skill", "--category", "-c", help="Skill category: skill, guide, template, script, course, consulting"),
...
def search(
    ...
    if HAS_RICH:
        ...
        for item in results:
            ...
            console.print(
                f"  [bold yellow]{item.get('title')}[/bold yellow]"
                f"  [dim]by {item.get('creator_name')}[/dim]"
                f"  APOL: [green]{score}[/green]  [{tags_str}]"
            )
...
@app.command()
def report() -> None:
    """Report a bug or issue — opens a pre-filled GitHub Issue in your browser."""
    import platform
    import urllib.parse
    import webbrowser
    from cli._constants import USER_AGENT
    ...
    if HAS_RICH:
        ...
        console.print(Panel(
            ...
            title="[bold magenta]zforge report[/]",
            border_style="magenta"
        ))
        console.print()
        console.print(f"[dim]CLI: {VERSION} | Python: {platform.python_version()} | OS: {platform.system()} {platform.release()}[/dim]")
        console.print()
        description = console.input("[bold yellow]Describe the issue (or press Enter to skip): [/bold yellow]").strip()
    else:
        print("\n=== ZeroForge Bug Report ===")
        print(f"CLI: {VERSION} | Python: {platform.python_version()} | OS: {platform.system()} {platform.release()}")
        description = input("\nDescribe the issue (or press Enter to skip): ").strip()

    # Build issue body
    body = "## Description\n\n"
    body += (description if description else "_Describe what happened here..._") + "\n\n"
    body += "## Steps to Reproduce\n\n1. \n2. \n3. \n\n"
    body += "## Expected Behavior\n\n\n\n"
    body += "## Actual Behavior\n\n\n\n"
    body += f"## System Info\n\n{sys_info}\n"
    body += "---\n_Generated by `zforge report`_\n"

    # Build GitHub Issue URL
    params = urllib.parse.urlencode({
        "title": f"[Bug] {description[:80]}" if description else "[Bug] ",
        "body": body,
        "labels": "bug",
    })
    url = f"https://github.com/frdel/agent-zero/issues/new?{params}"
    ...
    try:
        ...
        if HAS_RICH:
            console.print()
            console.print("[green]✔ Opened GitHub Issues in your browser![/green]")
            console.print("[dim]Fill in any extra details and submit.[/dim]")
        else:
            print("\nOpened GitHub Issues in your browser.")
    except Exception:
        if HAS_RICH:
            console.print()
            console.print("[yellow]Could not open browser. Copy this URL:[/yellow]")
            console.print(f"[cyan]{url}[/cyan]")
        else:
            print(f"\nCopy this URL to report: {url}")
